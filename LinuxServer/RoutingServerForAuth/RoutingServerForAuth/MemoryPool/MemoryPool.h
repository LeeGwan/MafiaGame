...
private:
  ...
  std::vector<std::vector<uint8_t>> packet_owned_buffers;  // 실제 메모리 소유
  std::mutex packet_pool_mutex;
  std::condition_variable packet_cv;

  // ClientPacket 메모리 풀
  std::queue<ClientPacket*> ClientPacket_pool_storage;
  std::vector<ClientPacket> ClientPacket_owned_buffers;  // 실제 메모리 소유
  ...
  std::vector<CompactBinaryReader> CompactBinaryReader_owned_buffers;  // 실제 메모리 소유
